        ...
        <div class="card card-outline card-success shadow border-0 rounded-lg overflow-hidden">
            <div class="card-header bg-gradient-success text-white py-3" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%);">
                <h3 class="card-title font-weight-bold mb-0">
                    <i class="fas fa-plus-circle mr-2"></i> Ongeza Aina ya Mtihani
                </h3>
            ...
            <form action="{{ route('admin.result-types.store') }}" method="POST">
                ...
                <div class="card-body p-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control form-control-lg shadow-sm @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="Mfano: Mock Exam">
                        <small class="text-muted">Jina hili litaonekana kwenye examination categories.</small>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="form-control shadow-sm @error('description') is-invalid @enderror" rows="3" placeholder="Maelezo mafupi...">{{ old('description') }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="card-footer bg-light text-right py-3 border-top">
                    ...
                    <button type="submit" class="btn btn-success px-5 rounded-pill shadow-sm">
                        <i class="fas fa-save mr-1"></i> Hifadhi Aina
                    </button>
                </div>
            </form>
        </div>
