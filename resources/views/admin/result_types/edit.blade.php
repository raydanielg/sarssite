            ...
            <form action="{{ route('admin.result-types.update', $resultType) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $resultType->name) }}" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description', $resultType->description) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="card-footer text-right">
                    ...
                    <button type="submit" class="btn btn-success">Update Aina</button>
                </div>
            </form>
        </div>
    </div>
