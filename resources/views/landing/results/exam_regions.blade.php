...
@section('title', $examName . ' ' . $yearData->year . ' - Chagua Mkoa')

@section('content')
<div class="breadcrumb-bar fixed top-0 left-0 w-full p-3 sm:p-4 z-50 flex flex-col gap-2">
    <div class="flex items-center gap-2 flex-wrap">
        <a href="{{ route('results.exam_years', $examSlug) }}" class="results-link inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 text-[#1e293b] text-xs sm:text-sm font-bold rounded-lg transition-all group">
            <i class="ri-arrow-left-line group-hover:-translate-x-1 transition-transform"></i> Rudi Miaka
        </a>
        <div class="flex items-center gap-1 text-[10px] sm:text-xs font-bold text-gray-500 px-3 py-1.5">
            ...
            <i class="ri-arrow-right-s-line"></i>
            <span class="text-gray-400">Chagua Mkoa</span>
        </div>
    </div>
</div>
...
<section class="pt-24 pb-12 sm:pt-32 min-h-screen flex items-center">
    <div class="container mx-auto px-4">
        <div class="text-center mb-8 animate__animated animate__fadeInDown">
            <h2 class="text-xl sm:text-2xl font-black text-[#1e293b]">{{ $examName }} - {{ $yearData->year }}</h2>
            <p class="text-xs text-gray-400 mt-2 uppercase tracking-widest font-bold">Chagua Mkoa</p>
        </div>
        ...
        <div class="results-card max-w-4xl mx-auto p-6 sm:p-8 animate__animated animate__fadeInUp">
            @if($regions->isNotEmpty())
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                    @foreach($regions as $index => $region)
                        <a href="{{ route('results.exam_districts', [$examSlug, $yearData->year, $region->slug]) }}"
                           ...
                           style="animation-delay: {{ $index * 0.05 }}s">
                            <div class="flex flex-col items-center">
                                <div class="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center group-hover:bg-green-600 transition-colors mb-2">
                                    <i class="ri-map-pin-line text-xl text-green-600 group-hover:text-white transition-colors"></i>
                                </div>
                                ...
                                <div class="mt-1 text-[10px] font-bold text-[#1f7a35] uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity">
                                    Chagua Wilaya <i class="ri-arrow-right-line"></i>
                                </div>
                            </div>
                        </a>
                    @endforeach
            ...
            @else
                <div class="text-center py-16 animate__animated animate__fadeIn">
                    <i class="ri-map-line text-5xl text-gray-300 mb-3"></i>
                    <p class="text-gray-400 font-bold uppercase tracking-widest text-sm">Hakuna mkoa uliopatikana.</p>
                </div>
            @endif
        </div>
    </div>
